...
try:
    __version__ = version("clishortener")
except PackageNotFoundError:
    __version__ = "Unknown (Installed from source)"
...
def initurlparser(servicedefault: str) -> argparse.ArgumentParser:
    ...
    servicecommand = subparsers.add_parser("service", help="Manage Shortening Services")
    serviceparser = servicecommand.add_subparsers(
        dest="servicecommand", metavar="", description="Allows Manageing Services"
    )
    servicecommand.set_defaults(func=lambda args, *_: servicecommand.print_help())
    defaultserviceparser = serviceparser.add_parser(
        "default",
        help=("Set Default Service | " "USAGE: cshorten " "service default <SERVICE>"),
    )
    defaultserviceparser.add_argument(
        ...
        help=(
            ...
            f" 'usrconfig.json' for future use"
            f" (Current {servicedefault})"
        ),
    )
    defaultserviceparser.set_defaults(func=handle_service_default)
    listparser = serviceparser.add_parser("list", help=("List All Available Services"))
    listparser.set_defaults(func=handle_service_list)
    securityparser = subparsers.add_parser(
        "security", help="Contains Commands Related To Security / OPSEC"
    )
    securitysubparser = securityparser.add_subparsers(
        dest="securitycommand", metavar=""
    )
    securityparser.set_defaults(func=lambda args, *_: securityparser.print_help())
    proxyparser = securitysubparser.add_parser(
        "proxy", help="Interactively Edit HTTP/HTTPS Proxy Configurations"
    )
    proxyparser.set_defaults(func=lambda args, *_: proxyparser.print_help())
    proxysubparsers = proxyparser.add_subparsers(dest="proxycommand", metavar="")
    proxyedit = proxysubparsers.add_parser("edit", help="Edit Proxy Configurations")
    proxyedit.set_defaults(func=handle_security_proxy_edit)
    proxyshow = proxysubparsers.add_parser(
        "show", help="Display Current Proxy Configurations"
    )
    proxyshow.set_defaults(func=handle_proxy_show)
    proxyreset = proxysubparsers.add_parser(
        "reset", help="Clear Saved Proxy Configurations"
    )
    proxyreset.set_defaults(func=handle_proxy_reset)
    shortenparser = subparsers.add_parser("shorten", help="Shortens URLS")
    shortenparser.add_argument(
        "url", action="store", nargs="?", default=None, help="Url that is shortened."
    )
    ...
    shortenparser.add_argument(
        ...
        "--qr",
        action="store_true",
        help="Generates a scannable qr code for your shortened link",
    )
    shortenparser.add_argument(
        "--no-strip",
        action="store_true",
    ...
    shortenparser.add_argument(
        ...
        "--alias",
        action="store",
        help=("Allows setting a custom alias/slug " "for shorteners that support it"),
    )
    shortenparser.add_argument(
        "-o",
        "--write-output",
        action="store",
        metavar="PATH",
        help="Allows appending links long and shortened version to a file"
    )
    ...
    advanced.add_argument(
        ...
        action="append",
        help=(
            "Allows setting temporary overrides " "for services.json key:value pairs."
        ),
    )
    ...
    keyringsubparser = keyringparser.add_subparsers(dest="keyringcommand", metavar="")
    keyringset = keyringsubparser.add_parser("set", help="Set a service API key.")
    keyringset.add_argument("service", help="Service that the key is set for.")
    ...
    parser.add_argument(
        "-S", "--silent", action="store_true", help="Silences all error messages"
    )
    parser.add_argument(
    "-v", "--version",
    action="version",
